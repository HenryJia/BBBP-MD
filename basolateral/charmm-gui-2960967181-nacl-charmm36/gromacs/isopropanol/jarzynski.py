...
# Step 1: get rid of the edge points by capping the free energy
fe[:, z < 1.0] = np.max(fe[:, (z > 1.0) * (z < 1.5)])
fe[:, z > 8.0] = np.max(fe[:, (z > 7.5) * (z < 8.0)])

# Step 2: Shift the free energy such that the maximum is at 0
fe -= np.max(fe)

# Step 3: Smooth the free energy using a kernel ridge regression
...
fe_smooth = np.mean(fe_smooth, axis=0)# - np.var(fe_smooth, axis=0)/(2.0*k_B*T)
